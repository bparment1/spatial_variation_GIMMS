...
def raster_copy_with_nodata( s_fh, s_xoff, s_yoff, s_xsize, s_ysize, s_band_n,
    ...
    s_band = s_fh.GetRasterBand( s_band_n )
    t_band = t_fh.GetRasterBand( t_band_n )
    cM_band = countMask.GetRasterBand( 1 )
    ...
    data_maskCount = cM_band.ReadAsArray( t_xoff, t_yoff, t_xsize, t_ysize )
    
    #Add overlapping area
    to_write = Numeric.where((data_dst == nodata) & (data_src == nodata),data_dst,data_dst+data_src)
    outMaskToWrite = Numeric.where((data_dst == nodata) & (data_src == nodata),data_maskCount,data_maskCount + 1)

    t_band.WriteArray( to_write, t_xoff, t_yoff )
    cM_band.WriteArray( outMaskToWrite, t_xoff, t_yoff )   
...
class file_info:
    ...
    def copy_into( self, t_fh, s_band = 1, t_band = 1, nodata_arg=None,countMask=None ):
        """
...
def main( argv=None ):
    ...
    while i < len(argv):
        ...
        if arg == '-o':
            ...
            #bType = "Float32"
            bType = "Float64"
            band_type = gdal.GetDataTypeByName( bType ) #argv[i] )
            print "Band type ",band_type 
            if band_type == gdal.GDT_Unknown:
                print('Unknown GDAL data type: %s' % bType) #argv[i])
                sys.exit( 1 )
    ...
    #DEBUG: Not sure why it's not working, but we need this 
    #       to prevent overflow.
    #band_type = gdal.GetDataTypeByName( "Float64" )
    ...
    if os.path.exists(tempFile):
      os.remove(tempFile)
    gdal.PushErrorHandler( 'CPLQuietErrorHandler' )
    countMask = gdal.Open( tempFile, gdal.GA_Update )
    gdal.PopErrorHandler()
    ...
    if t_fh is None:
        ...
        print band_type
        countMask = Driver.Create( tempFile, xsize, ysize, bands,
                              band_type, create_options )

        if countMask is None:
           print('Creation of mask failed')
           sys.exit(1)
            
        t_fh.SetGeoTransform( geotransform )
        t_fh.SetProjection( file_infos[0].projection )

        countMask.SetGeoTransform( geotransform )
        countMask.SetProjection( file_infos[0].projection )

        if copy_pct:
            t_fh.GetRasterBand(1).SetRasterColorTable(file_infos[0].ct)
            countMask.GetRasterBand(1).SetRasterColorTable(file_infos[0].ct)
    else:
        if separate != 0:
            bands=0
            for fi in file_infos:
    ...
    if a_nodata is not None:
        ...
        #countMask.GetRasterBand(1).SetNoDataValue(0)
    ...
    #Pre intialize to zero for mask
    #for i in range(countMask.RasterCount):
    countMask.GetRasterBand(1).Fill( 0 )
    ...
    for fi in file_infos:
        ...
        if separate == 0 :
            for band in range(1, bands+1):
                fi.copy_into( t_fh, band, band, nodata, countMask )
        else:
            for band in range(1, fi.bands+1):
                fi.copy_into( t_fh, band, t_band, nodata, countMask )
                t_band = t_band+1
    ...
    b1 = t_fh.GetRasterBand(1)
    outRas = b1.ReadAsArray()
    
    #Quick hack, We're just going to take the sum for this version
    '''
    mask = countMask.GetRasterBand(1).ReadAsArray()

    output = Numeric.where((mask > 0),(outRas/mask),0)
    #output[(output == -Numeric.inf) | (output == Numeric.inf)] = 0
    output = Numeric.where((output < -100) | (output > 100),0,output)
    output = output.astype(Numeric.float32)    
    '''

    #pylab.imshow(output,vmin=0,vmax=55.0)
    #pylab.show()
    #pylab.imshow(mask,vmin=0,vmax=2)
    #pylab.colorbar()
    #pylab.show()

    '''
    print outRas[1500:1510,4000]
    print mask[1500:1510,4000]
    print output[1500:1510,4000]

    print '----------'

    print output[2500:2510,7000]
    print mask[2500:2510,7000]
    print outRas[2500:2510,7000]

    print '----------'
    
    print output[1000:1010,1000]
    print mask[1000:1010,1000]
    print outRas[1000:1010,1000]
    '''

    b1 = t_fh.GetRasterBand(1)
    #b1.WriteArray(output,0,0)
    b1.WriteArray(outRas,0,0)
